export default class PitchNode extends AudioWorkletNode {
  ...
  // Handle an error caught in pitch processor
  ...
  onmessage(event) {
    if (event.type === 'wasm-module-loaded') {
			// if pitch detector is loaded compile the audio analysis to collect audio to detect pitch
      this.port.postMessage({
        type: "init-detector",
      ...
      // if pitch is detected then callback the pitch
